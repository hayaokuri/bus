...
def send_discord_notification(message):
    ...
    try:
        ...
        logging.info(f"Discord通知送信成功: {message[:50]}...") # INFOレベルなので通常は表示されない
    except requests.exceptions.RequestException as e:
        logging.error(f"Discord通知送信失敗: {e}") # ERRORレベルなので表示される
    except Exception as e:
        logging.error(f"Discord通知送信中に予期せぬエラー: {e}") # ERRORレベルなので表示される
...
def parse_bus_info_from_html(html_content):
    ...
    for wrap_element in bus_wrappers:
        ...
        departure_time_display = "情報なし"
        raw_status_text = ""
        parsed_delay_info = None
            ...
            if frameBox03:
                ...
                if departure_stop_area:
                    ...
                    if notes_span:
                        ...
                        if time_match_notes:
                            stop_specific_time = time_match_notes.group(1)
                            departure_time_display = f"{stop_specific_time}発" # 乗車バス停の時刻
...
def fetch_and_cache_bus_data(route_id, from_stop_no, to_stop_no_for_request, current_time_unix):
    ...
    if current_time_unix - active_bus_cache.get("timestamp", 0) > BUS_DATA_CACHE_DURATION_SECONDS \
       or not active_bus_cache.get("data_valid", False):
        logging.info(f"バス情報({route_id})を神奈中サイトから更新します。") # INFOレベルなので通常は表示されない
        ...
        try:
            ...
            logging.warning(f"バス情報取得タイムアウト ({route_id})")
            send_discord_notification(f"警告: バス情報取得がタイムアウトしました (ルートID: {route_id})")
        except requests.exceptions.RequestException as e:
            active_bus_cache["error"] = f"バス情報取得リクエストエラー: {e}"; active_bus_cache["data"] = []
            logging.error(f"バス情報取得リクエストエラー ({route_id}): {e}")
            send_discord_notification(f"エラー: バス情報取得でリクエストエラーが発生しました (ルートID: {route_id}): {e}")
        except Exception as e:
            active_bus_cache["error"] = f"バス情報処理中に予期せぬエラー: {e}"; active_bus_cache["data"] = []
            logging.exception(f"バス情報処理エラー ({route_id})")
            send_discord_notification(f"重大エラー: バス情報処理中に予期せぬエラーが発生しました (ルートID: {route_id}): {e}")
...
def api_data():
    ...
    current_dt_tokyo_for_api = datetime.datetime.now(TOKYO_TZ) # api_data 内で時刻を一度だけ取得
    try:
        requested_direction_group = request.args.get('direction_group', 'to_station_area')
        all_routes_bus_data = {}
        current_time_unix = time.time()
        combined_errors_for_group_api = [] # このAPIリクエストでのバス関連エラーをまとめる
        ...
        if requested_direction_group == 'to_station_area' or requested_direction_group == 'to_university_area':
            ...
            if requested_direction_group == 'to_station_area':
                 sanno_buses_for_filter, s_err = fetch_and_cache_bus_data("sanno_to_station", ROUTE_DEFINITIONS["sanno_to_station"]["from_stop_no"], ROUTE_DEFINITIONS["sanno_to_station"]["to_stop_no"], current_time_unix)
                 if s_err: combined_errors_for_group_api.append(f"大学発(重複チェック用): {s_err}")
            ...
            for route_key, route_config in routes_to_process_for_group:
                ...
                if error: combined_errors_for_group_api.append(f"{route_config['from_stop_name_short']}発: {error}") # APIレベルでのエラー集約
                ...
                for bus_info_original in buses_raw:
                    if route_key == "ishikura_to_station" and bus_info_original.get(KEY_VEHICLE_NO) and bus_info_original.get(KEY_VEHICLE_NO) in sanno_vehicle_numbers_if_applicable: continue
                    bus_info = bus_info_original.copy()
                    time_until_str, is_urgent, seconds_until, departure_dt = calculate_and_format_time_until(
                        bus_info.get(KEY_DEPARTURE_TIME, "情報なし"), bus_info.get(KEY_STATUS_TEXT, ""),
                        current_dt_tokyo_for_api, bus_info.get(KEY_DELAY_INFO)
                    )
                    bus_info.update({
                        KEY_TIME_UNTIL: time_until_str, KEY_IS_URGENT: is_urgent, KEY_SECONDS_UNTIL_DEPARTURE: seconds_until,
        ...
        try:
            ...
            if (current_dt_tokyo_for_api.hour == WEATHER_FETCH_HOUR and not weather_fetched_today_g) or (not weather_cache.get("data") and not weather_cache.get("error")):
                ...
                if error: send_discord_notification(f"警告: 天気情報の取得に失敗しました - {error}") # 天気取得エラーも通知
...
if __name__ == '__main__':
    ...
    app.run(host='0.0.0.0', port=port, debug=False) # 本番環境を想定し debug=False に変更
