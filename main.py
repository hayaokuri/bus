...
def get_weather_info(api_key, location_query):
    ...
    if not api_key or api_key == "YOUR_OPENWEATHERMAP_API_KEY_HERE":
        ...
        return None, None, None, "APIキー未設定" # 温度も返すように変更
...
@app.route('/')
def index():
    # 初期表示用に、APIが返すであろう情報を一部渡す
    # 実際のデータはJavaScriptで /api/data から取得する
...
def api_data():
    ...
    if current_hour == WEATHER_FETCH_HOUR and not weather_fetched_today_g:
        ...
        weather_cache["data"] = {"condition": condition, "description": description, "temp_c": temp, "is_rain": (condition and "rain" in condition.lower())} # "rain" in condition.lower() に修正
    ...
    if is_before_service_hours:
        ...
        bus_fetch_error = bus_data_cache.get("error")
         ...
         or not bus_data_cache.get("data") or not bus_data_cache.get("data_valid", True): # データ無効フラグも考慮
        ...
        bus_data_cache["data_valid"] = True # 取得成功したら有効
        if bus_result.get("error"):
            bus_data_cache["data_valid"] = False # エラー時は無効
    ...
    if not is_before_service_hours and not bus_fetch_error and not processed_buses:
        ...
        # ここで no_bus_info_count のようなものを導入する場合、セッションやDBでの管理が必要
    ...
    # app_state_message に基づく判定の調整
    if app_state_message not in ["監視中", "情報なし/運行終了の可能性"] and \
       not app_state_message.startswith("始発バス待機中"):
        # これらの状態は正常動作の範囲内とし、エラーがなければ healthy を維持
        if bus_fetch_error : # エラーがある場合のみ unhealthy
            system_healthy = False
