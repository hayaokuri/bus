                ...
                if (data.routes_bus_data.hasOwnProperty(displayGroupId)) {
                    ...
                    if (routeDisplayData.bus_error_message) { routeHtml += `<p class="error-message"><i class="fas fa-exclamation-circle"></i> バス情報取得エラー: ${routeDisplayData.bus_error_message}</p>`; }
                        ...
                        routeDisplayData.buses_to_display.forEach((bus, index) => {
                            ...
                            let ishikuraRelatedChipHtml = ''; // 石倉関連チップ
                            ...
                            if (currentDirectionGroup === 'to_university_area') {
                                if (bus.is_ishikura_stop_only) { // 純粋な石倉止まり
                                    itemAdditionalClass += ' ishikura-stop-target'; // 背景色などで区別するためのクラス
                                    ishikuraRelatedChipHtml = '<span class="location-badge ishikura-badge">石倉</span>';
                                    destinationDisplay = ''; // チップで「石倉」と示すので、メインの行き先表示は空に
                                } else if (bus.is_oyama_for_ishikura) { // 大山ケーブル行き（石倉経由）
                                    itemAdditionalClass += ' oyama-via-ishikura-target'; // 背景色などで区別するためのクラス
                                    ishikuraRelatedChipHtml = '<span class="location-badge ishikura-badge">石倉</span>'; // 石倉を経由することを示すチップ
                                    destinationDisplay = `${bus.destination_name}行`; // 元の行き先「大山ケーブル行」を表示
                                }
                                // 産業能率大学行きの場合は ishikuraRelatedChipHtml は空、destinationDisplay は「産業能率大学行」
                            }
